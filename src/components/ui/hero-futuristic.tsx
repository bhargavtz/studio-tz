...
export const HeroFuturistic = () => {
  ...
  return (
    <div className="h-svh relative bg-black overflow-hidden">
        ...
        <div className="text-3xl md:text-5xl xl:text-6xl 2xl:text-7xl font-extrabold">
          <div className="flex space-x-2 lg:space-x-6 overflow-hidden text-white">
            {titleWords.map((word, index) => (
              <div
                key={index}
                className={`transition-opacity duration-500 ${index < visibleWords ? 'opacity-100' : 'opacity-0'}`}
                style={{ 
      ...
      <div className="absolute bottom-20 left-0 right-0 flex justify-center z-50 pointer-events-auto">
        <Link href="/builder">
          <button
            className="group relative px-8 py-4 bg-transparent border border-white/20 text-white font-bold rounded-full hover:bg-white/10 transition-all duration-300 flex items-center gap-3 backdrop-blur-sm animate-in fade-in slide-in-from-bottom-4 fill-mode-forwards opacity-0"
